...
Claude Code（Proプラン）を使ってアプリ開発を進めていたところ、**5時間のレート制限**に達することが増えてきました。

「あと少しで完了するのに...」というタイミングで制限に達すると、作業が中断されるのが大きなストレスです。

この記事では、レート制限を回避しつつ効率的に開発を進めるための運用パターンをまとめます。

:::message
**筆者が特に効果を感じた3つの対策**（効果は環境や運用スタイルにより異なります）

1. **マルチエージェント環境の活用**（対策9）- Codex連携でClaude消費ゼロのレビュー
2. **Skills/Rulesの整備**（対策5）- コンテキストを最適化して消費を抑える
3. **`/usage`で定期的に監視**（対策1）- 制限に達する前に対策
:::
...
Claude Code Proプランには、UI上で表示される5時間の使用枠があります。

- **対象**: トークン消費量（`/usage`で確認可能）
- **リセット**: 5時間のローリング枠（UI表示に基づく）
- **超過時**: 一時的に利用制限

:::message alert
レート制限の詳細仕様は公式に公開されていないため、本記事の内容はUI表示と実際の運用経験に基づいています。仕様は変更される可能性があります。
:::
...
:::details インストール方法
詳細なインストール手順は[公式リポジトリ](https://github.com/thedotmack/claude-mem)を参照してください。
:::
...
サブエージェントは便利ですが、呼び出し回数が増えるほど消費が増えます。

### 解決策

- **自動呼び出しを控える** - 必要なときだけ明示的に呼び出す運用ルール
- **直接コマンドを優先** - `npm run check` を直接実行（サブエージェント経由にしない）
- **並列実行を意識する** - 複数の調査を同時に走らせすぎない

:::message
これらは機能設定ではなく、運用上の工夫です。サブエージェントの呼び出し頻度を意識することで消費を抑えられます。
:::
...
※ 5時間枠の中で複数回のセッションを行う想定です。

---

## 対策9: マルチエージェント環境でコンテキストを節約

### 概要

Claude Code単体で全てを処理するのではなく、**複数のエージェントに役割分担**させることで、メインエージェントのコンテキスト消費を抑えられます。
...
| 方法 | コンテキスト節約効果 | 説明 |
|------|---------------------|------|
| サブエージェント | ★★★ | 独立したコンテキストで動作、結果のみ返却 |
| Codex連携 | ★★★ | 別プロバイダーなのでClaude側の消費ゼロ |
| 並列プランニング | ★★☆ | 複数観点を一度に取得、往復を削減 |

### 1. サブエージェントで調査を分離

コードベースの探索や調査をサブエージェントに委譲すると、**メインエージェントのコンテキストに調査過程が残りません**。

```
❯ /ask-agent 認証機能がどこに実装されているか調べて

⏺ Explore エージェントが独立したコンテキストで調査
  - 複数ファイルを読み込み
  - grepで検索
  - 結果を要約
...
### 2. Codexでセカンドオピニオン（Claude消費ゼロ）

OpenAI Codex CLIを使うと、**Claude側のトークンを一切消費せず**にレビューや相談ができます。

:::message alert
`/codex`は標準コマンドではなく、外部ツール（Codex CLI）のインストールと設定が必要です。また、コード内容がOpenAI側に送信されるため、機密性の高いコードを扱う場合は注意してください。別途OpenAI側のコストも発生します。
:::
...
複数の観点を**一度に並列取得**することで、会話の往復を減らせます。

```
❯ 新機能Xの実装計画を立てて

⏺ Claude Code（メイン）が並列実行:
  ├─→ Plan エージェント: 実装戦略
  ├─→ Explore エージェント: 既存コード調査
  └─→ Codex: 設計レビュー

⏺ 3つの結果を統合して一度に報告
  → 「調べて」「次はこれ」「レビューして」の往復が不要
```

### スキルの設定方法

詳細な設定方法は別記事を参照:
https://zenn.dev/nenene01/articles/claude-code-multi-agent-skills

### 運用のコツ

| 場面 | 推奨アプローチ |
|------|---------------|
| コードベース調査 | `/ask-agent` でExploreエージェント |
| 実装前の計画 | `/ask-agent` でPlanエージェント |
| コードレビュー | `/codex` でセカンドオピニオン |
| 設計相談 | `/codex` で別視点を取得 |
| 複雑なタスク | 並列プランニングで一括処理 |
...
:::message
モデル名は世代更新により変更される可能性があります。サブエージェントのモデル設定は `.claude/agents/` 内の定義ファイルで行います。最新のモデル名は[公式ドキュメント](https://code.claude.com/docs/ja)を確認してください。
:::
...
**最も効果的な3つ:**
1. **マルチエージェント環境の活用** - Codex連携でClaude消費ゼロのレビュー、サブエージェントで調査を分離
2. Skills/Rules を整備してコンテキストを最適化
3. `/usage` で定期的に監視し、制限前に対策
